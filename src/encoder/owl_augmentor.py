...
class OwlPredictor(torch.nn.Module):
    ...
    def augment_image_class_embeds(self, image_class_embeds, logit_shift, logit_scale):
        #return torch.cat([image_class_embeds, logit_shift], axis=-1) * logit_scale
        #logit_shift = logit_shift.expand_as(image_class_embeds)
        #logit_scale = logit_scale.expand_as(image_class_embeds)
    ...
    def encode_image_torch(self, image: torch.Tensor) -> OwlEncodeImageOutput:
        ...
        # Apply shift and scale in advance, then normalize
        ...
        output = OwlEncodeImageOutput(
            ...
            image_class_embeds=image_class_embeds_aug,
    ...
    def encode_image_trt(self, image: torch.Tensor) -> OwlEncodeImageOutput:
        print("Encode image by tensorrt.")
    ...
    def decode(self, 
        ...
        image_class_embeds_aug = image_output.image_class_embeds
        ...
        print("Text dimension:", query_embeds_aug.shape)
        print("Image class dimension:", image_class_embeds_aug.shape)
        print("Pred boxes dimension:", image_output.pred_boxes.shape)
        print("Scale dimension:", image_output.logit_scale.shape, image_output.logit_shift.expand_as(image_output.image_class_embeds).shape)

        # Compute the logits
        logits = torch.einsum('...pd,...qd->...pq', image_class_embeds_aug, query_embeds_aug)
