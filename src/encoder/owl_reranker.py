...
class OwlEncodeImageOutput:
    ...
    image_class_embeds_aug: torch.Tensor
...
class OwlPredictor(torch.nn.Module):
    ...
    def augment_image_class_embeds(self, image_class_embeds, logit_shift, logit_scale):
        return (image_class_embeds + logit_shift) * logit_scale
    
    def augment_query_embeddings(self, query_embeds, target_dim):
        # Ensure the query embeddings have the same dimension as image_class_embeds
        current_dim = query_embeds.shape[-1]
        if current_dim < target_dim:
            pad_size = target_dim - current_dim
            query_embeds_aug = torch.cat([query_embeds, torch.ones_like(query_embeds[..., :pad_size])], dim=-1)
        else:
            query_embeds_aug = query_embeds
        return query_embeds_aug
    ...
    def encode_image_torch(self, image: torch.Tensor) -> OwlEncodeImageOutput:
        ...
        # Apply class embeds normalize, then shift and scale
        image_class_embeds = image_class_embeds / (torch.linalg.norm(image_class_embeds, dim=-1, keepdim=True) + 1e-6)
        image_class_embeds_aug = self.augment_image_class_embeds(image_class_embeds, logit_shift, logit_scale)

        # print(image_embeds.shape, image_class_embeds.shape)
        # print(logit_scale.shape, logit_shift.shape)

        output = OwlEncodeImageOutput(
            image_embeds=image_embeds,
            image_class_embeds=image_class_embeds,
            image_class_embeds_aug=image_class_embeds_aug,
    ...
    def decode(self, 
        ...
        # print(num_input_images, image_output.image_class_embeds.shape)
        image_class_embeds_aug = image_output.image_class_embeds_aug
        # image_class_embeds_aug = image_class_embeds_aug / (torch.linalg.norm(image_class_embeds_aug, dim=-1, keepdim=True) + 1e-6)

        query_embeds = text_output.text_embeds
        query_embeds_aug = self.augment_query_embeddings(query_embeds, image_class_embeds_aug.shape[-1])
        # query_embeds_aug = query_embeds_aug / (torch.linalg.norm(query_embeds_aug, dim=-1, keepdim=True) + 1e-6)
        
        # Ensure query_embeds_aug and image_class_embeds_aug have the same dimension
        if query_embeds_aug.shape[-1] != image_class_embeds_aug.shape[-1]:
            raise ValueError(f"Dimension mismatch: query_embeds_aug has dimension {query_embeds_aug.shape[-1]}, "
                            f"but image_class_embeds_aug has dimension {image_class_embeds_aug.shape[-1]}")

        # print(query_embeds.shape, image_class_embeds.shape)

        # Compute the logits
        logits = torch.einsum('...pd,...qd->...pq', image_class_embeds, query_embeds)
        
        # print(logits.shape,image_output.logit_shift.shape, image_output.logit_scale.shape)
        ...
        # print(self.image_size, num_input_images, self.num_patches)
        ...
        # print(input_indices.shape, mask.shape)
        # print(image_output.pred_boxes.shape)
    ...
    def export_image_encoder_onnx(self, 
        ...
        if use_dynamic_axes:
            dynamic_axes =  {
                ...
                "image_class_embeds_aug": {0: "batch"},
                "logit_shift": {0: "batch"},
                "logit_scale": {0: "batch"},
                "pred_boxes": {0: "batch"}       
            }
        ...
        class Wrapper(torch.nn.Module):
            ...
            def forward(self, image):
                ...
                return OwlEncodeImageOutput(
                    ...
                    image_class_embeds_aug=torch.cat([r[2] for r in results], dim=0),
                    logit_shift=torch.cat([r[3] for r in results], dim=0),
                    logit_scale=torch.cat([r[4] for r in results], dim=0),
                    pred_boxes=torch.cat([r[5] for r in results], dim=0)
                )
    ...
    def image_encoder_milvus(self, 
        ...
        image_encodings_output = self.encode_rois(image_tensor, rois, pad_square=pad_square)
        return image_encodings_output
